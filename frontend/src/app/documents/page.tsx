...
/* ─────────────── DATA ─────────────── */
const documents = [
  { id: 1, type: "Income Certificate", issuer: "Revenue Dept. Govt. of Maharashtra", date: "12 Oct 2024", expires: "12 Oct 2027", status: "verified", usedIn: ["PM KISAN", "PMAY-G"] },
  { id: 2, type: "Aadhaar / Citizen Identity", issuer: "UIDAI – Unique Identification Authority of India", date: "05 Jan 2018", expires: "Lifetime", status: "verified", usedIn: ["All Schemes"] },
  { id: 3, type: "Land Ownership Record (7/12)", issuer: "Revenue Dept. Govt. of Maharashtra", date: "15 Mar 2026", expires: "15 Mar 2027", status: "verified", usedIn: ["PM KISAN", "PM FBY"] },
  { id: 4, type: "Caste Certificate (OBC)", issuer: "Social Welfare Dept. Govt. of Maharashtra", date: "–", expires: "Pending", status: "pending", usedIn: ["PM FBY"] },
  { id: 5, type: "12th Marksheet / Student ID", issuer: "Maharashtra State Board of Education", date: "10 Aug 2023", expires: "Expired", status: "revoked", usedIn: [] },
];

function StatusBadge({ status }: { status: string }) {
  if (status === "verified") return <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded bg-[#ECFDF5] text-[#065F46] border border-[#A7F3D0]"><CheckCircle2 className="w-3.5 h-3.5" />Verified</span>;
  if (status === "pending") return <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded bg-[#FFFBEB] text-[#92400E] border border-[#FDE68A]"><Clock className="w-3.5 h-3.5" />Pending</span>;
  return <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded bg-[#FEF2F2] text-[#991B1B] border border-[#FECACA]"><XCircle className="w-3.5 h-3.5" />Revoked</span>;
}
      ...
      <main id="main" className="flex-1 max-w-7xl w-full mx-auto px-6 py-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div>
            ...
            <p className="text-[#64748B] text-[14px] mt-1.5">Cross-department credentials verified by government authorities.</p>
          </div>
          <button className="flex items-center gap-2 text-white font-semibold px-5 py-2.5 rounded-sm hover:opacity-90 transition-opacity text-sm shadow-brand" style={{ background: "#0B3CC8" }}>
                  ...
                  <tr key={doc.id} className={`border-t border-[#E2E8F0] hover:bg-[#F8FAFC] transition-colors ${doc.status === "revoked" ? "bg-[#FEF2F2]/40" : ""}`}>
                    <td className="px-8 py-5">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="font-semibold text-[#0F172A] text-[14.5px] leading-snug">{doc.type}</div>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-5">
                      <div className="flex flex-wrap gap-1.5">
                        {doc.usedIn.length > 0
                          ? doc.usedIn.map(s => <span key={s} className="text-[10.5px] font-semibold bg-[#EEF3FF] border border-[#0B3CC8]/20 px-2 py-0.5 rounded text-[#0B3CC8]">{s}</span>)
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-6">
            {[
              ...
              <div key={i} className="flex items-center gap-6 w-full md:w-auto">
                ...
                {i < 2 && (
                  <ArrowRight className="w-6 h-6 text-[#CBD5E1] hidden md:block shrink-0" />
                )}
              </div>
            ))}
          </div>
